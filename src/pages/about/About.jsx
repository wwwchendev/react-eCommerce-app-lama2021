...
export const About = () => {
  const sideItems = [
    {
      title: '品牌介紹',
      linkUrl: '/about?type=brand',
      contentElement: <Brand />,
    },
    { title: '購物說明', linkUrl: '/about?type=qa', contentElement: <QA /> },
    {
      title: '售後服務',
      linkUrl: '/about?type=afterSales',
      contentElement: <AfterSales />,
    },
    {
      title: '隱私權保護',
      linkUrl: '/about?type=privatePolicy',
      contentElement: <PrivatePolicy />,
    },
  ]
  ...
  const currentItem =
    sideItems.find(item => item.linkUrl.includes(`type=${type}`)) ||
    sideItems[0]
  ...
  return (
    <Layout.PageLayout>
      <SEO
        title={`${currentItem.title} | RESTART-SHOP`}
        description={null}
        url={null}
      />
      <StyledContainer>
