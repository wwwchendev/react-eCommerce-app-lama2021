...
const showPriceRange = (product) => {
  ...
  const maxPrice = product.variants.reduce((max, variant) =>
    variant.price > max ? variant.price : max, product.variants[0].price);

  const minPrice = product.variants.reduce((min, variant) =>
    variant.price < min ? variant.price : min, product.variants[0].price);

  const text = maxPrice === minPrice
...
export const Product = () => {
  ...
  const [currentSpecificationIndex, setCurrentSpecificationIndex] = useState(null);
  const maxQuantity = product?.variants[currentVariantIndex]?.specification[currentSpecificationIndex]?.stock
  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        const response = await customAxios.get(`${import.meta.env.VITE_APIURL}/product/${id} `);
  ...
  useEffect(() => {
    if (product !== null) {
      const productCategory = product?.productCategories?.find((item) => item.categoryType === "依類別")
      if (productCategory) { setCategoryName(productCategory?.categoryName) }
      setPaths([
        { label: '首頁', path: '/' },
        { label: '商品列表', path: '/products' },
        {
  ...
  const handleSubmit = async e => {
    ...
    if (variant && specification) {
      ...
      dispatch(cartRequests.addCartItem(TOKEN, authUserState.data.username, data))
      setIsAddedCart(true)
  ...
  const handleLike = () => {
    ...
    if (!isDuplicate) {
      ...
      dispatch(likedProductRequests.addLikedItem(TOKEN, authUserState.data.username, data))
    } else {
  ...
  const removeLike = () => {
    ...
    const data = { productId: product._id }
    dispatch(likedProductRequests.removeLikedItem(TOKEN, authUserState.data.username, data))
  }
  ...
  return (
    <Layout.PageLayout>
      ...
      <StyledContainer>
            ...
            {product && <>
              <ProductInfo container justifyContent='center' spacing={3}>
                <Grid item xs={12} md={6}>
                  <ProductImageBox>
                    <ProductImageSlider images={[product.productPromotionImage, ...product.productImages]} />
                  </ProductImageBox>
                </Grid>
                <Grid item xs={12} md={6}>
                  <ProductDetails>
                    ...
                    <PriceAndAddLikedWrapper>
                      ...
                      <AddToLikedProductsButton
                        ...
                        $isAdded={isDuplicate}
                      >
                        {isDuplicate ? <> <HeartTick />已收藏</> : <> <HeartAdd />收藏</>}
                      </AddToLikedProductsButton>
                    </PriceAndAddLikedWrapper>
                    <ProductDetail>
                    ...
                    <ProductDetail>
                      ...
                      >
                        {
                          product.variants[currentVariantIndex || 0].specification.map((item, idx) => {
                            return <OptionButton
                              type="button"
                              key={item._id}
                              onClick={() => { setCurrentSpecificationIndex(idx) }}
                    ...
                    <ProductDetail>
                      ...
                      <QuantityWrapper>
                        <NumberInput
                          ...
                          $size={"6rem"}
                        />
                        <p>{maxQuantity ? `剩餘數量 ${maxQuantity} 件` : ""}</p>
                      </QuantityWrapper>
              ...
              <Content>
                {HTMLReactParser(product.richContent)}
              </Content>
            </>
            }
          </Grid>
        ...
        {showAlert && <Alert title="加入購物車" iconType={"info"} onClose={() => setShowAlert(false)} >
          請選擇規格與尺寸
        </Alert>}
        {showSuccess && <Alert title="加入購物車" iconType={"cartCheck"} onClose={() => setShowSuccess(false)} >
          已加入購物車
        </Alert>}
        {showCartLoginConfirm && <Confirm title="添加至購物車" iconType={"info"}
          onConfirm={() => {
            navigate('/login')
        ...
        {showLikedLoginConfirm && <Confirm title="添加至收藏清單" iconType={"info"}
          onConfirm={() => {
            navigate('/login')
