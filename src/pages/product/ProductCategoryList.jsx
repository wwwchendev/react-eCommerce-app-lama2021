...
const ProductCategoryList = ({ categoryName }) => {
  ...
  const categoryByPopular = data.filter(item => item.type === "依群組" && item.categoryName === "熱銷推薦");
  ...
  return (
    <Container>
      ...
      <Section>
        ...
        <Title $mb="1rem">
          <StyledLink to={`/products?category=熱銷推薦`}
      ...
      <Section>
        ...
        <List>
          <ListItem $active={release === "202406"}>
            <StyledLink to={`/products?release=20240609`}
              $active={release === "20240609"}>
              20240609
            </StyledLink>
          </ListItem>
          <ListItem $active={release === "20240523"}>
            <StyledLink to={`/products?release=20240523`}
      ...
      <Section>
        <Title><StyledLink to={`/products`}>商品分類</StyledLink></Title>
            ...
            categoryList.map((group, index) => {
              ...
              return (
                <ListItem $active={category === group.category} key={index}>
                  <CategoryWrapper>
                    <StyledLink to={`/products?category=${group.category}`}
                      $active={(category === group.category || categoryName === group.category)}
                    >
                      {group.category}
                    </StyledLink>
                    <IconButton
                  ...
                  </CategoryWrapper>
                  {
                    group.subCategory.map((sub, subIndex) => {
                      return (
                        <SubList $open={showDropdown[group.category]} key={subIndex} $active={subCategory === sub}>
                          <ListItem $active={subCategory === sub}>
                            <StyledLink to={`/products?category=${group.category}&subCategory=${sub}`}
