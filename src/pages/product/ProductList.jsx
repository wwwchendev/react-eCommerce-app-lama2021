  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        if (category) {
          updatedPaths.push({ label: category, path: `/products?category=${category}` });
          if (!subCategory) {
            response = await customAxios.post(`${import.meta.env.VITE_APIURL}/product/paginated`, {
              page: page,
            ...
            updatedPaths.push({ label: subCategory, path: `/products?category=${category}&subCategory=${subCategory}` });
          }
        } else if (search) {
          response = await customAxios.post(`${import.meta.env.VITE_APIURL}/product/paginated`, {
            page: page,
          ...
          updatedPaths.push({ label: `搜尋結果【${search}】`, path: `/products?query=${category}` });
        } else if (release) {
          response = await customAxios.post(`${import.meta.env.VITE_APIURL}/product/paginated`, {
            page: page,
            categoryName: "依上架日期",
          ...
          updatedPaths.push({ label: release, path: `/products?release=${release}` });
        } else {
          response = await customAxios.post(`${import.meta.env.VITE_APIURL}/product/paginated`, {
            page: page,
            sort: "createdAt",
        ...
        <Grid container justifyContent='center' spacing={3}>
          ...
          <Grid item xs={12} md={10}>
            <SorterWidget>
              ...
              {products.length > 0
                ? <p>顯示 {perPage * (page - 1) + 1} - {page !== totalPages ? perPage * page : totalDataCount} 共 {totalDataCount} 項商品</p>
