...
export const Checkout = () => {
  ...
  const totalCount = cartState.data?.products?.reduce((sum, item) => sum + item.quantity, 0);//計算件數總和
  useEffect(() => {
    if (!cartItems) {
  ...
  const handleFormChange = e => {
    ...
    if (name === 'zipcode' || name === 'county' || name === 'district' || name === 'address' || name === 'convenienceStore') {
  ...
  //handleSubmit
  const handleSubmit = () => {
    const { username, status, products, productSubtotal, logistic, shippingDiscount, total, payment, memo } = form
  ...
  useEffect(() => {
    if (operateType === 'createOrder') {
      if (orderState.error === null) {
        ...
        dispatch(cartRequests.clearAllCartItems(TOKEN, authUserState.data.username))
      } else {
        alert('新增時出現錯誤')
      }
    }
  ...
  useEffect(() => {
    if (form.logistic?.receiver?.receiverName !== "" && form.logistic?.receiver?.receiverMobileNumber !== "") {
      if (form.logistic?.option === "超商取貨-全家" ||
        form.logistic?.option === "超商取貨-711") {
        if (!form.logistic?.address?.convenienceStore?.storeName || !form.logistic?.address?.convenienceStore?.storeId) {
          setCompletedTodo(p => ({ ...p, shippingMethod: false }))
          return
        }
      }
      if (!form.logistic?.address?.zipcode || !form.logistic?.address?.county || !form.logistic?.address?.district || !form.logistic?.address?.address) {
        setCompletedTodo(p => ({ ...p, shippingMethod: false }))
        return
      } else {
        ...
        <Grid container direction='column' justifyContent='center' alignItems='center' >
          ...
          <Grid container justifyContent='center' spacing={3} >
            ...
            <Grid item xs={12} lg={8} >
              <Main>
                      ...
                      <tbody>
                        {cartState.data.products.map((item, idx) => (
                          <tr key={idx}>
                            <td>
                              <ProductInfo>
                                ...
                                <div>
                                  ...
                                  <SmInfoWrapper>
                                    <p>{item.variantName} {item.specificationName}</p>
                                    <div>
                                      <p>單價 NT.{numberWithCommas(item.price)}</p>
                                      <p>共 {numberWithCommas(item.quantity)} 件</p>
                            ...
                            <td data-label="合計">
                              <p>NT ${numberWithCommas(item.price * item.quantity)}</p>
                            </td>
                          </tr >
                        ))}
                      </tbody >
                ...
                <ShippingMethod>
                  ...
                  <Row>
                    <Col>
                      <InputWrapper>
                        ...
                        >
                          <option value="" disabled={true}>選擇物流配送方式</option>
                          <option value="宅配到府">宅配到府 /$150</option>
                          <option value="超商取貨-711">超商取貨-711 /$45</option>
                          <option value="超商取貨-全家">超商取貨-全家 /$45</option>
                        </select>
                      </InputWrapper>
                    </Col>
                  </Row>
                  ...
                  {form.logistic?.option === '宅配到府' && (
                    <HomeDeliveryWrapper>
                      <Field>
                        <RadioWrapper>
                          <input type="radio" id="defaultAddress"
                            ...
                            checked={
                              form.logistic?.option === '宅配到府'
                              && form.logistic?.address?.option === "defaultAddress"
                            }
                          />
                      ...
                      <Field>
                        <RadioWrapper>
                          <input type="radio" id="anotherAddress"
                            name="homeDeliveryOption"
                            value="anotherAddress"
                            onChange={handleFormChange}
                            checked={
                              form.logistic?.option === '宅配到府'
                              && form.logistic?.address?.option === "anotherAddress"
                            }
                          />
                        ...
                        <AddressWrapper>
                          <Row >
                            <Col>
                              <Row >
                                <Col>< Input type="text" name="zipcode" placeholder='郵遞區號'
                                  value={form.logistic?.address?.option === "anotherAddress" ? form.logistic?.address?.zipcode : ""}
                                  onChange={handleFormChange}
                                  disabled={form.logistic?.address?.option === "defaultAddress"}
                                /></Col>
                                <Col>< Input type="text" name="county"
                                  placeholder='縣市'
                                  value={form.logistic?.address?.option === "anotherAddress" ? form.logistic?.address?.county : ""}
                                  onChange={handleFormChange}
                                  disabled={form.logistic?.address?.option === "defaultAddress"}
                                /></Col>
                                <Col>< Input type="text" name="district"
                                  placeholder='鄉鎮市區'
                                  value={form.logistic?.address?.option === "anotherAddress" ? form.logistic?.address?.district : ""}
                                  onChange={handleFormChange}
                                  disabled={form.logistic?.address?.option === "defaultAddress"} /></Col>
                              </Row>
                              <Row >
                                <Col>< Input type="text" name="address" placeholder='路段巷弄號碼'
                                  value={form.logistic?.address?.option === "anotherAddress" ? form.logistic?.address?.address : ""}
                                  onChange={handleFormChange}
                                  disabled={form.logistic?.address?.option === "defaultAddress"} /></Col>
                              </Row>
                            </Col>
                  ...
                  {(form.logistic?.option === '超商取貨-711' || form.logistic?.option === '超商取貨-全家') && (
                    <HomeDeliveryWrapper>
                      <Field>
                        <p>選擇門市</p>
                        <AddressWrapper>
                          <Row >
                            <Col>
                                ...
                                <Col>
                                  <InputWrapper>
                                    <select
                                      ...
                                      value={form.logistic?.address?.option === "anotherAddress" ? form.logistic?.address?.convenienceStore?.storeId : ""}
                                      onChange={handleFormChange}
                                      disabled={!form.logistic?.address?.district}
                                    >
                              ...
                              <Row >
                                ...
                                <Col>< Input type="text" name="convenienceStoreAddress" placeholder='路段巷弄號碼'
                                  value={form.logistic?.address?.option === "anotherAddress" ? form.logistic?.address?.address : ""}
                                  disabled={true} /></Col>
                  ...
                  <Row>
                    ...
                    <Col>
                      <InputWrapper>
                        <p>電話</p>
                        <input type="text" name="receiverMobileNumber"
                ...
                <PaymentMethod>
                  <h3>2.支付方式</h3>
                  <div>
                    <input type="radio" name="payment" id="線上付款" value="線上付款" checked={form.payment.type === "線上付款"} onChange={handleFormChange} />
                    <label htmlFor="線上付款">線上付款</label>
                  </div>
                  <div>
                    <input type="radio" name="payment" id="貨到付款" value="貨到付款" checked={form.payment.type === "貨到付款"} onChange={handleFormChange} />
                ...
                <InvoiceMethod>
                  <h3>3.發票開立方式</h3>
                  <RadioWrapper>
                    <input type="radio" name="invoiceOption" id="紙本發票" value="紙本發票" checked={form.payment?.invoice?.type === "紙本發票"} onChange={handleFormChange} />
                    <label htmlFor="紙本發票">紙本發票</label>
                  </RadioWrapper>
                  <Field>
                    <RadioWrapper>
                      <input type="radio" name="invoiceOption" id="發票載具" value="發票載具" checked={form.payment?.invoice?.type === "發票載具"} onChange={handleFormChange} />
                      <label htmlFor="發票載具">發票載具</label>
                    </RadioWrapper>
                    <Input type="text" name="carrierNumber" id="" onChange={handleFormChange} value={form.payment?.invoice?.carrierNumber}
            ...
            <Grid item xs={12} lg={4} >
              <TotalAmount >
                {/* <pre>
                  {JSON.stringify(form.logistic, null, 2)}
                </pre> */}
                <Subtotal>
                  <p><span>{totalCount}</span>件商品，商品金額</p>
                  <p>NT$ {numberWithCommas(totalAmount)}</p>
                </Subtotal>
                <Subtotal>
                  <p>物流費用</p>
                  <p>{form?.logistic?.option} NT${numberWithCommas(form?.logistic?.fee || 0)}</p>
                </Subtotal>
                <Subtotal>
                  <p>總 計</p>
                  <p>NT$ <span>{numberWithCommas(totalAmount + form?.logistic?.fee)}</span></p>
                </Subtotal>
                ...
                <CheckList>
                  ...
                  <AgreementWrapper>
                    <IconWrapper onClick={() => {
                      setCompletedTodo(p => ({ ...p, agreement: !p.agreement }))
                    }}>
                      {completedTodo.agreement
                        ? <CheckSquare />
                        : <CheckBlank />}
                    </IconWrapper>
                    <p>我同意辦理退貨時，依照restart-shop
