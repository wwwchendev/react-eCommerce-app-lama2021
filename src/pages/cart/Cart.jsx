...
export const Cart = () => {
  ...
  const handleRemoveToLikedFromCart = item => {
    ...
    //加入收藏
    dispatch(
      likedProductRequests.addLikedItem(TOKEN, authUser?.username, {
        'productId': item.productId,
      }),
    )
    //移除商品
  ...
  useEffect(() => {
    ...
    if (
      operateType === 'removeToLikedFromCart' &&
                          ...
                          <Variant>
                            ...
                            <NumberInputWrapper>
                              <NumberInput
                                ...
                                disableCondition={cartState.loading}
                              />
                            </NumberInputWrapper>
                          </Variant>
                          <PriceWrapper>
      ...
      <SummarizeWrapper>
        <SummarizeContainer>
          <div>
            <p>
              總金額(不含運費):{' '}
              <span>NT$ {numberWithCommas(totalAmount || 0)}</span>{' '}
            </p>
          </div>
          <CheckoutButton
            onClick={() => {
