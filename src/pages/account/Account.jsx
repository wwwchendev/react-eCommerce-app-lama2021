...
export const Account = () => {
  ...
  const handleVerify = async e => {
    ...
    if (formInput.password) {
      ...
      const verifyUser = async () => {
        const response = await customAxios.get(`${import.meta.env.VITE_APIURL}/user/${authUserState.data.username}`, { headers: { Authorization: `Bearer ${TOKEN}` } });
  ...
  const handleSubmit = async e => {
    ...
    if (firstName && lastName && email && mobile && address.zipcode && address.county && address.district && address.address) {
      setLoading(true)
      const updateUser = async () => {
        ...
        try {
          const response = await customAxios.put(`${import.meta.env.VITE_APIURL}/user/${authUserState.data.username}`, updatedData, { headers: { Authorization: `Bearer ${TOKEN}` } });
          await dispatch(AuthUserRequests.refreshData(TOKEN, authUserState.data.username))
          setShowSuccess(true)
        } catch (error) {
  ...
  return (
    <Layout.PageLayout>
      ...
      <StyledContainer>
        ...
        {showSuccess && <Alert title="修改會員資料" iconType={"success"} onClose={() => setShowSuccess(false)} > 資料已修改
        </Alert>}
        <Grid container justifyContent='center' >
          ...
          <Grid item xs={12} md={10} style={{ border: '1px solid red' }}>
            <Grid container justifyContent='center' style={{ border: '1px solid red' }}>
              ...
              <Grid item xs={12} md={7} style={{ border: '1px solid red' }}>
                {
                  verified
                    ? <FormWrapper>
                      ...
                      <Form>
                        <Row>
                          <Col>
                            ...
                            <InputWrapper>
                              <Input type='text' name="username" placeholder='用戶名稱' onChange={handleChange} id="用戶名稱" $border={promptMessage.username} value={formInput.username}
                                disabled={true} />
                            </InputWrapper>
                          </Col>
                        </Row>
                        <Row>
                          <Col>
                            ...
                            <InputWrapper $offsetX={"4rem"}>
                              <Input type='text' name="lastName" placeholder='姓氏' onChange={handleChange} id="姓氏" $border={promptMessage.lastName} value={formInput.lastName} />
                          ...
                          <Col>
                            ...
                            <InputWrapper $offsetX={"4rem"}>
                              <Input type='text' name="firstName" placeholder='名字' onChange={handleChange} id="名字" $border={promptMessage.firstName} value={formInput.firstName} />
                          ...
                          <Col>
                            ...
                            <InputWrapper>
                              <Input
                                ...
                                id="生日"
                                $border={promptMessage.birthdate} value={formInput.birthdate}
                                disabled={true}
                              />
                        ...
                        <Row>
                          <Col>
                            ...
                            <InputWrapper $offsetX={"4rem"}>
                              <Input type='email' name="email" placeholder='電子郵件' onChange={handleChange} id="email" $border={promptMessage.email} value={formInput?.email} />
                        ...
                        <Row>
                          <Col>
                            ...
                            <InputWrapper $offsetX={"4rem"}>
                              <Input type='phone' name='mobile' placeholder='行動電話' onChange={handleChange} id="行動電話" $border={promptMessage.mobile} value={formInput?.mobile} />
                          ...
                          <Col>
                            ...
                            <InputWrapper>
                              <Input type='phone' name='phone' placeholder='市話(可選)' onChange={handleChange} id="市話" $border={promptMessage.phone} value={formInput?.phone} />
                        ...
                        <Row>
                          <Col>
                            ...
                            <InputWrapper >
                              <Input type='zipcode' name='zipcode' placeholder='郵遞區號' onChange={handleChange} id="郵遞區號" $border={promptMessage.address?.zipcode} value={formInput?.address?.zipcode} />
                              <span>{promptMessage?.address?.zipcode}</span>
                            </InputWrapper>

                            <InputWrapper>
                              <Input type='county' name='county' placeholder='縣市' onChange={handleChange} id="縣市" $border={promptMessage.address?.county} value={formInput?.address?.county} />
                              <span>{promptMessage.address?.county}</span>
                            </InputWrapper>

                            <InputWrapper>
                              <Input type='district' name='district' placeholder='鄉鎮市區' onChange={handleChange} id="鄉鎮市區" $border={promptMessage.address?.district} value={formInput?.address?.district} />
                        ...
                        <Row>
                          <Col>
                            <label htmlFor=""> </label>
                            <InputWrapper>
                              <Input type='address' name='address' placeholder='詳細地址路段樓層' onChange={handleChange} id="詳細地址" $border={promptMessage.address?.address} value={formInput?.address?.address} />
           ...
           我同意根據 <b>隱私權政策</b> 處理個人資料
                    ...
                    : <FormWrapper>
                      ...
                      <Form>
                        ...
                        <Col>
                          <InputWrapper >
                            <Input type='password' name='password' onChange={handleChange} autoComplete='current-password' placeholder='輸入密碼進行驗證(必要)' id="密碼" $border={promptMessage.password} />
