...
import { numberWithCommas, getDayString, getDateString, getTimeString } from '@/utils/format.js';
...
export const Orders = () => {
  ...
  const filterOrders = (orders, status) => {
    ...
    return orders.filter(order => {
      switch (status) {
        case '待付款':
          return order.status === '已成立' && order.payment.status === '尚未付款';
        case '待出貨':
        ...
        case '退貨退款':
          return order.status === '退貨作業中' || order.status === '退貨退款' || order.status === '退貨異常';
        default:
          return true;
      }
    });
          ...
          <Grid item xs={12} md={10} style={{ border: '1px solid red' }}>
            {(orderState.loading) ? (
              ...
              <ButtonContainer>
                ...
                >
                  待付款 <span>({filterOrders(orderState.data, '待付款').length})</span>
                </Button>
                <Button
                  onClick={() => setFilterStatus('待出貨')}
                  $actived={filterStatus === '待出貨'}
                >
                  待出貨 <span>({filterOrders(orderState.data, '待出貨').length})</span>
                </Button>
                <Button
                  onClick={() => setFilterStatus('待取貨')}
                  $actived={filterStatus === '待取貨'}
                >
                  待取貨 <span>({filterOrders(orderState.data, '待取貨').length})</span>
                </Button>
                <Button
                  onClick={() => setFilterStatus('已完成')}
                  $actived={filterStatus === '已完成'}
                >
                  已完成 <span>({filterOrders(orderState.data, '已完成').length})</span>
                </Button>
                <Button
                  onClick={() => setFilterStatus('取消')}
                  $actived={filterStatus === '取消'}
                >
                  取消 <span>({filterOrders(orderState.data, '取消').length})</span>
                </Button>
                <Button
                  onClick={() => setFilterStatus('退貨退款')}
                  $actived={filterStatus === '退貨退款'}
                >
                  退貨退款<span>({filterOrders(orderState.data, '退貨退款').length})</span>
                </Button>
              </ButtonContainer>
              <TableContainer>
                <Table>
                  <thead>
                    <tr>
                      <th scope="col" width="12.5%">訂單編號</th>
                      <th scope="col" width="12.5%">訂單日期</th>
                      <th scope="col" width="10%">合計</th>
                      <th scope="col" width="10%">訂單狀態</th>
                      <th scope="col" width="18%">款項狀態</th>
                  ...
                  <tbody>
                    {filteredOrders.length > 0 ? (
                      filteredOrders.map(order => {
                        return <tr key={order.orderNumber} onClick={() => { navigate(`/orders/${order.orderNumber}`) }}>
                          <td data-cell="訂單編號">
                            {order.orderNumber}
                          </td>
                          <td data-cell="訂單日期">
                          ...
                          <td data-cell="訂單狀態">
                            <div>
                              <p>{order.status}</p>
                              {order.completedAt && <span> {getDayString(new Date(order.completedAt), "-")}</span>}
                            </div>
                          </td>
                          <td data-cell="款項狀態">
                            <span> {order.payment.type} / {order.payment.status}</span>
                          </td>
                          <td data-cell="訂單商品">
                            <OrderProductImages>
                              ...
                              <span>共{numberWithCommas(reduceCalculate(order.products, "quantity"))}項</span>
                            </OrderProductImages>
                            <ViewButton>
                              檢視
                              <Search />
