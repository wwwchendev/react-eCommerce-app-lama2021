...
export const Login = () => {
  ...
  useEffect(() => {
    const storedLoginState = JSON.parse(localStorage.getItem('btnClicked'))?.loginRequest;
    if (storedLoginState) {
  ...
  const handleLogin = async e => {
    ...
    if (username && password) {
      ...
      await dispatch(AuthUserRequests.login({
            ...
            <Grid item xs={12} sm={10} md={7} lg={5} >
              <FormWrapper >
                ...
                <Form >
                  ...
                  <FormFooter>
                    {loginClicked && <PromptMessage>{authUserState?.error?.login}</PromptMessage>}
                    <SubmitButton
                      type="button"
