...
export const SingleNews = () => {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      setLoading(true);
      try {
        const response = await customAxios.get(`${import.meta.env.VITE_APIURL}/news/${title}`);
  ...
  return (
    <Layout.PageLayout>
      ...
      <StyledContainer>
        ...
        <Bottom>
          {navItems.prev && <Navigate to={`/news/${navItems.prev?.title}`}
            disabled={navItems.prev} $type="prev">
            <div><ArrowBack />上一則</div>
            <p>{navItems.prev?.title}</p>
          </Navigate>}

          {navItems.next && <Navigate to={`/news/${navItems.next?.title}`} $type="next">
            <div>下一則<ArrowForward /></div>
