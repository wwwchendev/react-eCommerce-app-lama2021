...
padding:${(props) => (props.$viewMode === 'grid' ? '0.5rem 1rem' : '2rem 2rem')}; 
width: 100%;
height: 100%;
  display: flex;
  ...
  gap: 8px;
  ${sm({
  padding: (props) => (props.$viewMode === 'grid' ? '0.3rem 1rem' : '1.5rem 1.5rem')
})}
${sm({
  padding: (props) => (props.$viewMode === 'grid' ? '0.3rem 1rem' : '1rem 1rem')
})}
`
  ...
  return (
    <>
      <ProductCardContainer $viewMode={$viewMode}>
        <ImageContainer $viewMode={$viewMode} to={`/products/${product.productNumber}`}>
          <img src={`${import.meta.env.VITE_APIURL}/file${product.productPromotionImage}`} alt={product.productName} />
        </ImageContainer>
        <ProductInfo $viewMode={$viewMode}>
          <ProductName to={`/products/${product.productNumber}`} $viewMode={$viewMode}>
            {product.productName}
          </ProductName>
          <ProductNumber>{product.productNumber}</ProductNumber>
          <ProductDescription to={`/products/${product.productNumber}`} $viewMode={$viewMode}>
            {product.description}
          </ProductDescription>
          <SelectComponent $viewMode={$viewMode} product={product} onChange={handleSpecificationChange} />
          <ProductDetails>
            <ProductPrice $viewMode={$viewMode}>{showPrice()}</ProductPrice>
            <ButtonContainer>
              {location.pathname === "/likedProducts" ? (<>
                <IconButton icon={HeartRemove}
                  onClick={() => handleRemoveLike(product._id)} />
                <IconButton icon={Cart3} onClick={() => {
