...
export const ProductCards = ({ $viewMode, products }) => {
  ...
  //驗證登入與商品是否已收藏
  const verifyLogin = useCallback(({ title }) => {
  ...
  //從商品卡片可以操作的通用功能
  //加入購物車
  const handleAddToCart = useCallback((selectedSpecification) => {
    ...
    setOperateType('cart');
    dispatch(cartRequests.addCartItem(TOKEN, authUserState.data.username, selectedSpecification));
  }, [TOKEN, verifyLogin]);
  ...
  const handleRemoveToCartFromLiked = (selectedSpecification) => {
    ...
    dispatch(cartRequests.addCartItem(TOKEN, authUserState.data.username, selectedSpecification))
    //移除商品
    dispatch(likedProductRequests.removeLikedItem(TOKEN, authUserState.data.username, { productId: selectedSpecification.productId }))
  }


  //加入收藏
  const handleAddLike = useCallback((productId) => {
  ...
  //移除收藏  
  const handleRemoveLike = useCallback((productId) => {
  ...
  useEffect(() => {
    if ((operateType === 'cart' && cartState.error === null) || (operateType === 'removeToCartFromLiked' && cartState.error === null && likedProductState.error === null)) {
      setShowAlert({
        iconType: 'cartCheck',
        title: '購物車',
        content: '已加入購物車',
